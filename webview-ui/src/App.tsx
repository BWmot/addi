  ...
  return (
    <div className="container" style={{ display: 'flex', gap: '20px' }}>
      <div className="left-col" style={{ flex: 1, minWidth: '300px' }}>
        ...
        {item.type === 'model' && (
          <ModelForm data={item.data as any} mode={formMode} parentId={item.parentId} isBatchMode={item.isBatchMode} batchCount={item.batchCount} parentProviderType={(item.data as any).parentProviderType} />
        )}
      </div>
    </div>
  );
