...
export const ProviderForm: React.FC<ProviderFormProps> = ({ data, mode }) => {
  ...
  const handleOptionChange = (field: keyof NonNullable<ProviderConfig['options']>, value: unknown) => {
    if (mode === 'read') return;
    setFormData(prev => ({
      ...prev,
    ...
    <div id="provider-form">
      ...
      {mode !== 'read' && (
        <div className="button-row">
          ...
          <button type="button" onClick={handleDelete} className="secondary-btn" style={{color: 'var(--vscode-errorForeground)'}}>{t('common.delete')}</button>
