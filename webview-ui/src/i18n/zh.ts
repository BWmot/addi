...
const zh: I18nMessages = {
  ...
  provider: {
    ...
    budgetTokensLabel: 'Budget Tokens',
  ...
  model: {
    ...
    thinkingHintMap: {
      'openai-responses':
        'OpenAI (Responses API)：映射到 reasoningEffort。',
      'openai-completions':
        'OpenAI 兼容 (DeepSeek, MiMo 等)：作为推理力度参数传递给支持思考的模型。',
      'anthropic-messages':
        'Anthropic：低→1024，中→4096，高→8192 tokens。映射到扩展思考预算。',
      'google-generateContent':
        'Google：映射到 Gemini 模型的 thinkingConfig.thinkingLevel。',
    } as Record<string, string>,
